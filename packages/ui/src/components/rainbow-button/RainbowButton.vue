...
<template>
  <component
    ...
    :class="
      cn(
        `
          rainbow-button group relative inline-flex h-11 cursor-pointer
          ...
          bg-size-[200%] [background-clip:padding-box,border-box,border-box]
          bg-origin-border px-8 py-2 font-medium text-primary-foreground
          transition-colors [border:calc(0.08*1rem)_solid_transparent]
          before:pointer-events-none before:absolute before:bottom-[-20%]
          before:left-1/2 before:z-0 before:h-1/5 before:w-3/5
          before:-translate-x-1/2
          before:bg-[linear-gradient(90deg,var(--color-1),var(--color-5),var(--color-3),var(--color-4),var(--color-2))]
          before:bg-size-[200%] before:filter-[blur(calc(0.8*1rem))]
