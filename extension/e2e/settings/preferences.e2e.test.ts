...
describe("theme and profile preferences", { concurrent: false }, () => {
  ...
  it("changes the recent header count from 1 to 20 and persists it across reloads", async () => {
    ...
    await expect.poll(() => count.inputValue()).toBe("3");
    const decrement = settings.getByTestId("settings-recentlyAddedCount-decrement");
    const increment = settings.getByTestId("settings-recentlyAddedCount-increment");
    await decrement.click();
    await expect.poll(() => count.inputValue()).toBe("2");
    await increment.click();
    await expect.poll(() => count.inputValue()).toBe("3");
    await count.fill("0");
    await count.press("Tab");
    await expect.poll(() => count.inputValue()).toBe("1");
    expect(await decrement.isDisabled()).toBe(true);
    await expect.poll(() => shortcuts.count()).toBe(2);
    await count.fill("21");
    await count.press("Tab");
    await expect.poll(() => count.inputValue()).toBe("20");
    expect(await increment.isDisabled()).toBe(true);
